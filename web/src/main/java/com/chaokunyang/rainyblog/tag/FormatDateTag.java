package com.chaokunyang.rainyblog.tag;
...
public class FormatDateTag extends TagSupport {
    ...
    static {
        try {
            GET_LOCALE_METHOD = SetLocaleSupport.class.getDeclaredMethod("getFormattingLocale", PageContext.class, boolean.class, boolean.class);
    ...
    private String formatDate(Date value, Locale locale) {
        ...
        if(type == Type.DATE || type == Type.BOTH) {
        ...
        if(this.type == Type.TIME || this.type == Type.BOTH) {
            DateFormat format = DateFormat.getDateInstance(this.timeStyle.getDateFormat(), locale);
            format.setTimeZone(this.timeZone);
    ...
    private String formatDate(TemporalAccessor value, Locale locale) {
        ...
        if(this.type == Type.TIME || this.type == Type.BOTH)
            dateFormat = DateTimeFormatter.ofLocalizedTime(this.timeStyle.getFormatStyle()).withLocale(locale).withZone(zoneId)::format;
    ...
    private Locale getLocale() throws JspTagException {
        try {
            return (Locale) GET_LOCALE_METHOD.invoke(null, this.pageContext, this, true, true); // If the underlying method is static, then the specified obj argument is ignored. It may be null.
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new JspTagException(e.toString(), e);
        }
    }
    ...
    private static enum Style {
        ...
        Style(int dateFormat, FormatStyle formatStyle) {
